...
/**
 * La classe checkGiocata si occupa di controllare la validita' di una parola
 * inserita nel gioco
 */
public class checkGiocata {
    ...
    /**
     * Controlla la validita' di una parola inserita formata dal set di lettere
     * passato come parametro
     * 
     * @param letters La lista di lettere che formano la parola.
     * @param giocate Il numero di giocate effettuate.
     * @return Un array di stringhe contenente lo stato della risposta e
     *         informazioni aggiuntive.
     *         - response[0]: "ok" se la parola e' valida, "error" altrimenti.
     *         - response[1]: "" se la parola e' valida, un messaggio di errore
     *         altrimenti.
     ...
     */
    public static String[] checkParola(ArrayList<letter> letters, int giocate)
        ...
        if (vocabolario.cercaParola(parola)) {
            // Se esiste controllo che sia scritta seguendo una linea orizzontale o
            // verticale
        ...
        // Se ha passato i primi controlli, verifico che la parola intersechi una
        // casella di un altra parola gia' presente
        if (temp) {
            // La parola deve intersecare una casella solo se non e' la prima parola
            // inserita
            if (giocate > 0) {
        ...
        if (!checkBuchi(letters)) {
            temp = true;
        } else {
            temp = false;
            response[1] = "Non puoi lasciare buchi nella parola!";
        }
    ...
    /**
     * Controlla che la parola sia scritta seguendo una linea orizzontale o
     * verticale
     * 
     * @param letters La lista di lettere che formano la parola.
     * @return true se la parola e' scritta seguendo una linea orizzontale o
     *         verticale, false altrimenti.
     */
     ...
     * @throws SAXException                 Se si verifica un errore SAX.
     */
    private static String calcolaPunteggio(ArrayList<letter> l)
            throws ParserConfigurationException, IOException, SAXException {
        ArrayList<letter> values = fileManager.getLetterValues();
        Integer punteggio = 0;
        // Ciclo all'interno delle lettere passate come parametro e sommo il valore di
        // ogni lettera preso dal file xml
        for (letter letter : l) {
    ...
    /**
     * Controlla se ci sono buchi nella parola
     * Un buco e'una lettera mancante in una sequenza consecutiva di lettere sia in
     * verticale che in orizzontale
     * 
     * @param l La lista di lettere fornita
     * @return true se ci sono buchi, false se non ci sono
     */
    private static boolean checkBuchi(ArrayList<letter> l) {
        boolean isBucata = false;
        for (int i = 0; i < l.size(); i++) {
            letter tmp = l.get(i);
            if (i < l.size() - 1) {
                int x = 0;
                x = Math.abs(tmp.getP().getX() - l.get(i + 1).getP().getX());
                if (x > 1) {
                    isBucata = true;
                    break;
                }
        ...
        for (int i = 0; i < l.size(); i++) {
            ...
            if (i < l.size() - 1) {
                int y = 0;
                y = Math.abs(tmp.getP().getY() - l.get(i + 1).getP().getY());
                if (y > 1) {
                    isBucata = true;
                    break;
                }
