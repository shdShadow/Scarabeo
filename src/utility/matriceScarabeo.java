...
public class matriceScarabeo extends JFrame {
    ...
    public matriceScarabeo() {
        ...
        for (int riga = 0; riga < 17; riga++) {
            for (int colonna = 0; colonna < 17; colonna++) {
                ...
                grigliaDiGioco[riga][colonna].setBackground(Color.GREEN); // Set the background color to green
    ...
    public void aggiungiLettera(int riga, int colonna, char lettera, int valore, player player, punto p) {// metodo per
                                                                                                          // aggiungere
                                                                                                          // lettere
                                                                                                          // nella
                                                                                                          // posizione
    ...
    private void mostraGriglia() {
        for (int row = 0; row < 17; row++) {
            for (int col = 0; col < 17; col++) {
                ...
                if (letteraCorrente != null) {

                    grigliaDiGioco[row][col].setText(Character.toString(letteraCorrente.getCharacter()));
                    grigliaDiGioco[row][col].setBackground(Color.GREEN);// set della
                                                                        // lettera
                                                                        // premuta
                                                                        // nella cella
                                                                        // selezionata
                ...
                if (letteraCorrente != null) {
                    if (letteraCorrente.getPlayer() != null) { // controlla se la lettera ha un giocatore associato
                        if (turnoGiocatore1 && letteraCorrente.getPlayer().getPlayer_name().equals("Player 1")) {
                            grigliaDiGioco[row][col].setForeground(Color.RED);// se la lettera appartiene al giocatore 1
                                                                              // la coloro di rosso
                        } else if (!turnoGiocatore1
                                && letteraCorrente.getPlayer().getPlayer_name().equals("Player 2")) {
                            grigliaDiGioco[row][col].setForeground(Color.BLUE); // se la lettera appartiene al giocatore
                                                                                // 2 la coloro di blu
                        }
                    }
                }
